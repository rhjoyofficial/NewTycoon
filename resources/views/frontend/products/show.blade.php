            ...
            <div class="grid lg:grid-cols-2">
                ...
                <div class="p-5 sm:p-7 lg:p-8 border-b lg:border-b-0 lg:border-r border-gray-100">
                    @php
                        ...
                        if (empty($images)) $images = [asset('images/placeholder.jpg')];
                    @endphp
                    ...
                    <div class="relative bg-gray-50 rounded-xl overflow-hidden mb-4 group" style="aspect-ratio:1/1;">
                        ...
                        @if ($product->compare_price > $product->price)
                            <div class="absolute top-3 left-3 pointer-events-none">
                                <span class="inline-flex items-center bg-primary text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
                        ...
                        @if ($product->stock_status !== 'in_stock')
                            <div class="absolute inset-0 bg-white/60 backdrop-blur-[1px] flex items-center justify-center">
                                <span class="bg-gray-800 text-white text-xs font-bold px-4 py-2 rounded-full tracking-wide">
                                    OUT OF STOCK
                                </span>
                            </div>
                        @endif
                ...
                <div class="p-5 sm:p-7 lg:p-8 flex flex-col gap-5">
                    ...
                    <div>
                        @if ($product->sku)
                            <p class="text-[11px] text-gray-400 uppercase tracking-widest mb-2">SKU: {{ $product->sku }}</p>
                        @endif
                    ...
                    <div class="flex items-end gap-3 flex-wrap">
                        ...
                        @if ($product->compare_price > $product->price)
                            ...
                            <span class="text-xs bg-red-50 text-red-600 border border-red-200 px-2.5 py-1 rounded-full font-semibold">
                                Save <span class="font-bengali">৳</span>{{ number_format($product->compare_price - $product->price, 0) }}
                            </span>
                        @endif
                    </div>
                    ...
                    <div>
                        @if ($product->stock_status === 'in_stock')
                            <span class="inline-flex items-center gap-2 text-sm font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-full">
                        ...
                        @else
                            <span class="inline-flex items-center gap-2 text-sm font-medium text-red-700 bg-red-50 border border-red-200 px-3 py-1.5 rounded-full">
                    ...
                    @if ($product->stock_status === 'in_stock')
                        <div class="space-y-3.5">
                            ...
                            <div class="flex flex-col sm:flex-row gap-3">
                                <form action="{{ route('cart.add', $product->id) }}" method="POST" class="flex-1" id="addCartForm">
                                    @csrf
                                ...
                                <form action="{{ route('checkout.buy-now', $product->id) }}" method="POST" class="flex-1">
                                    @csrf
                    ...
                    @if ($product->category || $product->model_number || $product->warranty_period)
                        <dl class="border-t border-gray-100 pt-4 grid grid-cols-[auto_1fr] gap-x-6 gap-y-2.5 text-sm">
                            ...
                            @if ($product->warranty_period)
                                ...
                                <dd class="font-medium text-gray-800">{{ $product->warranty_period }} {{ $product->warranty_type }}</dd>
                            @endif
                        </dl>
                    @endif
        ...
        <div id="tabs" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <nav class="flex gap-0 overflow-x-auto no-scrollbar -mb-px" aria-label="Product information tabs">
                    ...
                    <button type="button" data-tab-btn="reviews"
                        ...
                        @if ($product->rating_count > 0)
                            <span class="ml-1.5 text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">{{ $product->rating_count }}</span>
                        @endif
                    </button>
                </nav>
            </div>
            ...
            <div class="p-5 sm:p-7 lg:p-8">
                ...
                <div data-tab-panel="specs" class="hidden">
                    @php
                        $specs = array_filter($product->specifications_array ?? [], fn($s) => !empty($s['key']) && !empty($s['value']));
                        $hasSpecs = $product->sku || $product->model_number || $product->weight || count($specs) > 0;
                    @endphp
                    @if ($hasSpecs)
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    ...
                                    @if ($product->model_number)
                                        <tr class="odd:bg-gray-50/60">
                                            ...
                                            <td class="py-3 px-5 text-gray-900 font-medium">{{ $product->model_number }}</td>
                                        </tr>
                                    @endif
                                    @if ($product->weight)
                                        <tr class="odd:bg-gray-50/60">
                                            <td class="py-3 px-5 text-gray-500 font-medium w-2/5 sm:w-1/3">Weight</td>
                                            <td class="py-3 px-5 text-gray-900 font-medium">{{ $product->weight }} kg</td>
                                        </tr>
                                    @endif
                                    @foreach ($specs as $spec)
                                        <tr class="odd:bg-gray-50/60">
                                            <td class="py-3 px-5 text-gray-500 font-medium w-2/5 sm:w-1/3">{{ $spec['key'] }}</td>
                ...
                <div data-tab-panel="reviews" class="hidden">
                        ...
                        <div class="flex items-center gap-5 mb-8 p-5 bg-gray-50 rounded-xl">
                            <div class="text-center min-w-[80px]">
                                ...
                                <div class="flex justify-center gap-0.5 mb-1">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <svg class="w-3.5 h-3.5 {{ $i <= round($product->average_rating) ? 'fill-yellow-400' : 'fill-gray-200' }}" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                        </svg>
                                    @endfor
                                </div>
                                <p class="text-xs text-gray-500">{{ $product->rating_count }} {{ Str::plural('review', $product->rating_count) }}</p>
                            </div>
                        </div>
                                ...
                                <div class="border border-gray-100 rounded-xl p-5 hover:border-gray-200 transition-colors">
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="flex items-center gap-3 min-w-0">
                                            <div class="w-9 h-9 bg-primary/10 rounded-full flex items-center justify-center text-primary font-bold text-sm shrink-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div class="flex gap-0.5 mt-0.5">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        <svg class="w-3 h-3 {{ $i <= $review->rating ? 'fill-yellow-400' : 'fill-gray-200' }}" viewBox="0 0 20 20">
                                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                        </svg>
                                                    @endfor
                                                </div>
                    ...
                    @else
                        <div class="text-center py-14">
                            ...
                            <p class="text-xs text-gray-400">Be the first to share your experience with this product</p>
                        </div>
                    @endif
                </div>
                        ...
                        <a href="{{ route('product.show', $related->slug) }}"
                           class="group bg-white rounded-xl border border-gray-200 hover:border-primary/50 hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col">
                            ...
                            <div class="p-3 flex flex-col flex-1">
                                <h3 class="text-xs sm:text-sm font-medium text-gray-900 group-hover:text-primary transition-colors line-clamp-2 mb-2 flex-1 leading-snug">
                                    {{ $related->name }}
                                </h3>
                                <div class="flex items-baseline gap-1.5 flex-wrap">
                                    ...
                                    @if ($related->compare_price > $related->price)
                                        <span class="text-xs text-gray-400 line-through">
                                            <span class="font-bengali">৳</span>{{ number_format($related->compare_price, 0) }}
                                        </span>
                                    @endif
                                </div>
                            </div>
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (btnMinus) btnMinus.addEventListener('click', function () { syncQty(+qtyInput.value - 1); });
        if (btnPlus)  btnPlus.addEventListener('click',  function () { syncQty(+qtyInput.value + 1); });
        if (qtyInput) qtyInput.addEventListener('input', function () { syncQty(this.value); });
